...
export const Result = {
	 ...
	 * ```
	 */
	success: <TValue>(value: TValue): Success<TValue> =>
	 ...
	 * ```
	 */
	failure: <TError>(error: TError): Failure<TError> =>
		({ _tag: "failure", error }) as const,
	 ...
	 * ```
	 */
	fromThrowable: <TValue, TError = Error>(
		fn: () => TValue,
		errorMapper?: (error: unknown) => TError,
	): ResultType<TValue, TError> => {
		try {
	 ...
	 * ```
	 */
	isSuccess: <TValue, TError>(
		result: ResultType<TValue, TError>,
	): result is Success<TValue> => result._tag === "success",
	 ...
	 * ```
	 */
	isFailure: <TValue, TError>(
		result: ResultType<TValue, TError>,
	): result is Failure<TError> => result._tag === "failure",
	...
	): ResultType<TNewValue, TError> =>
		Result.isSuccess(result) ? Result.success(fn(result.value)) : result,
	...
	): ResultType<TValue, TNewError> =>
		Result.isFailure(result) ? Result.failure(fn(result.error)) : result,
	...
	flatMap: <TValue, TError, TNewValue>(
		...
		Result.isSuccess(result) ? fn(result.value) : result,
	...
	unwrapOr: <TValue, TError>(
		result: ResultType<TValue, TError>,
		defaultValue: TValue,
	): TValue => (Result.isSuccess(result) ? result.value : defaultValue),
	...
	): TValue => (Result.isSuccess(result) ? result.value : fn(result.error)),
	 ...
	 */
	all: <TValue, TError>(
		results: readonly ResultType<TValue, TError>[],
	): ResultType<TValue[], TError> => {
		const values: TValue[] = [];
	...
	fromPromiseAll: async <TValue, TError = Error>(
		...
		const results = await Promise.all(
			promises.map((p) => Result.fromPromise(p, errorMapper)),
		);
	 ...
	 * ```
	 */
	tapError: <TValue, TError>(
		result: ResultType<TValue, TError>,
		fn: (error: TError) => void,
	): ResultType<TValue, TError> => {
		if (Result.isFailure(result)) {
			fn(result.error);
		}
